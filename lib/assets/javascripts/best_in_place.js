...
BestInPlaceEditor.prototype = {
  ...
  update: function () {
    ...
    if (
      (this.oldValue.toLowerCase().includes(this.warning_if_old_value) ||
        this.oldValue === null) &&
      this.warning_before_save &&
      (!this.warning_before_save_with_new_values ||
        this.warning_before_save_with_new_values.indexOf(value) >= 0) &&
      !confirm(this.warning_before_save)
    ) {
      console.log("old value", typeof this.oldValue);
  ...
  initOptions: function () {
    ...
    self.element.parents().each(function () {
      ...
      self.okButtonClass =
        self.okButtonClass || $parent.data("bipOkButtonClass");
      self.cancelButton = self.cancelButton || $parent.data("bipCancelButton");
      self.cancelButtonClass =
        self.cancelButtonClass || $parent.data("bipCancelButtonClass");
      self.skipBlur = self.skipBlur || $parent.data("bipSkipBlur");
    });

    // Load own attributes (overrides all others)
    self.url =
      self.element.data("bipUrl") || self.url || document.location.pathname;
    ...
    self.attributeName =
      self.element.data("bipAttribute") || self.attributeName;
    self.activator = self.element.data("bipActivator") || self.element;
    self.okButton = self.element.data("bipOkButton") || self.okButton;
    self.okButtonClass =
      self.element.data("bipOkButtonClass") ||
      self.okButtonClass ||
      BestInPlaceEditor.defaults.okButtonClass;
    self.cancelButton =
      self.element.data("bipCancelButton") || self.cancelButton;
    self.cancelButtonClass =
      self.element.data("bipCancelButtonClass") ||
      self.cancelButtonClass ||
      BestInPlaceEditor.defaults.cancelButtonClass;
    self.skipBlur =
      self.element.data("bipSkipBlur") ||
      self.skipBlur ||
    ...
    self.original_content =
      self.element.data("bipOriginalContent") || self.original_content;

    self.warning_before_save =
      self.element.data("bipWarningBeforeSave") || self.warning_before_save;
    self.warning_before_save_with_new_values =
      self.element.data("bipWarningBeforeSaveWithNewValues") ||
      self.warning_before_save_with_new_values;
    self.warning_if_old_value =
      self.element.data("bipWarningIfOldValue") || self.warning_if_old_value;
    ...
    if (self.formType === "select" || self.formType === "checkbox") {
      ...
      self.collectionValue =
        self.element.data("bipValue") || self.collectionValue;
    }
  },
  ...
  select: {
    activateForm: function () {
      ...
      var output = jQuery(document.createElement("form"))
          ...
          .attr("style", "display:inline"),
        selected = "",
        select_elt = jQuery(document.createElement("select")).attr(
          "class",
